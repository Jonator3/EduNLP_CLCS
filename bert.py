...
def prepare_data(dataset: List[CrossLingualDataEntry], lang="og"):
    data = {}
    data["id"] = [d.id for d in dataset]
    data["Text"] = [d.get_text(lang) for d in dataset]
    ...
    def train(self, trainingset, kfold=0, verbose=False):
        ...
        if kfold > 0:
            ...
            for train_index, test_index in kf.split(trainingset):
                ...
                # TODO
                self.model, self.tokenizer = train_bert.train()
            ...
            # TODO
            self.model, self.tokenizer = train_bert.train(prepare_data(trainingset, self.lang), "Text", "score")
            return [], []
